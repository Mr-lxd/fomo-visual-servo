#!/usr/bin/env python3
"""Run the Slice 1 RoboBeetle LIVE Vision service on Raspberry Pi."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        logging.info(
            "Vision LIVE started source=%s observed=%sx%s fps=%s fourcc=%s bind=%s:%s",
            args.source,
        ...
        while True:
            ...
            logging.info(
                "Vision diag captured=%s latest_frame_id=%s client=%s "
                "selected=%s sent=%s skipped_ids=%s avg_jpeg_bytes=%s",
